...
- [ ] Select the "Video Review" tab in the main Connect window (the leftmost icon)
...
When reviewing the video it can be helpful to zoom into a bow ball of a boat. This done by placing the mouse cursor on the point of interest, pressing the left mouse button and dragging the mouse down. This can be repeated a few time to achieve the desired amount of zoom:
...
Because boats move at fairly constant speed, time from two different video frames can be used to calculate a finish time. This is useful when two boats finish very closely (and exact time is critical) and not one frame provides a view of the bow crossing the line exactly. This is done by following these steps:
...
By double clicking the image anywhere, it will reset the blue lines.
