...
| RaceType          |                    | 'Sprint', 'Head', 'Pursuit' or 'Info'.   'Sprint', 'Head' and 'Pursuit' indicate the type of timing. <br/><br/>'Info' declares an informational item in the schedule such as a break or lunch.   This allows a regatta with mixed Time Trials (head timing) and Sprint races. If unspecified, the global setting in the admin control for the regatta is used. Requires mobile app version 5.0.7 or later. <br/><br/>Pursuit races use handicap info to stagger the start so the highest handicap starts first with lowest handicap starting last.  When used in conjunction with a CrewTimer horn, the horn will sound for each start.  In addition, a minimum pursuit group interval can be configured to group starts together.  e.g. an interval of 2s would group handicaps that are within 2 seconds of each other so they start together. |
